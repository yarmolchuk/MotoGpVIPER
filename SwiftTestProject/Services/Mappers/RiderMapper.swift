...
private final class RiderMapperImpl: RiderMapper {
    func map(_ data: Data) -> [Rider]? {
        ...
        return json["items"].arrayValue.map({
             Rider(uid: $0["uid"].stringValue,
                   name: $0["name"].stringValue,
                   team: $0["team"].stringValue,
                   bike: $0["bike"].stringValue,
                   placeOfBirth: $0["placeOfBirth"].stringValue,
                   dateOfBirth: $0["dateOfBirth"].stringValue,
                   weight: $0["weight"].stringValue,
                   height: $0["height"].stringValue,
                   photoUrl: $0["photoUrl"].stringValue,
                   teamUid: $0["teamUid"].stringValue
        )})
    }
}
